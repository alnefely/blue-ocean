...
@section('title', __('trans.global.edit').': '.$admin->name)
    ...
    <div class="card-body">
        <form action="{{ url('/admin/admins/'.$admin->id) }}" method="post">@csrf {{ method_field('PUT') }}
            ...
            <div class="row">
                ...
                @if($admin->main=='normal')
                    <div class="col-md-4 mb-3">
                        <label class="form-label">{{__('trans.admin.role')}} <b class="text-danger">*</b></label>
                        <select class="select2 form-select" name="role_id" required data-allow-clear="true">
                            @foreach ($roles as $role)
                                <option @if($admin->role_id==$role->id) selected @endif value="{{$role->id}}">{{$role->name}}</option>
                            @endforeach
                        </select>
                    </div>
                @else
